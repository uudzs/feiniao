...
{
    $cate = Db::name('category')->field('id,name as title,pid,status')->where(['status' => 1])->order('ordernum asc')->select()->toArray();
    return $cate;
}
...
function get_adspace()
{
    ...
    $list = Db::name('adver')->field('id,title,create_time,update_time,channel as pid')->where(['status' => 1])->order('create_time asc')->select()->toArray();
