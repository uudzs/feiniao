...
class Auth
{
    public function handle($request, \Closure $next)
    {
        ...
        if ($pathInfo == '' || $action == '') {
            redirect((string) Route::buildUrl('index/index'))->send();
            exit;
        }
        //验证用户登录
        if ($action !== 'login' && $action !== 'verify') {
            ...
            if (!Session::has($session_admin)) {
                if ($request->isAjax()) {
                    ...
                    redirect((string) Route::buildUrl('login/index'))->send();
                    exit;
                }
            }
            $uid = Session::get($session_admin)['id'];
            // 验证用户访问权限
            if ($action !== 'index' && $action !== 'api' && $action !== 'addons' && $action !== 'upgrade') {
                if (!$this->checkAuth($controller, $pathInfo, $action, $uid)) {
                    if ($request->isAjax()) {
                        return to_assign(202, '你没有权限,请联系超级管理员！');
                    } else {
                        echo '<div style="text-align:center;color:red;margin-top:20%;">您没有权限,请联系超级管理员</div>';
                        exit;
                    }
                }
            }
    ...
    protected function checkAuth($controller, $pathInfo, $action, $uid)
    {
        ...
        $fChar = substr($pathUrl, 0, 1);
        if ($fChar == '/') {
            $pathUrl = substr($pathUrl, 1);
        }
        if (isset($domain_bind['admin']) && $domain_bind['admin']) {
            if (strpos($pathUrl, 'admin/admin/') !== false) {
                $pathUrl = str_ireplace('admin/admin/', 'admin/', $pathUrl);
            }
            if (!in_array($pathUrl, $skip, true)) {
                if (strpos($pathUrl, 'admin/') !== false) {
            ...
            foreach ($auth_list as $k => $v) {
                ...
                if (strpos($v, "admin/") !== false) {
                    $auth_list[$k] = str_ireplace('admin/', '', $v);
                }
            }
            $auth_list[] = 'index/main';
        } else {
            if (strpos($pathUrl, 'admin/admin/') !== false) {
                $pathUrl = str_ireplace('admin/admin/', 'admin/', $pathUrl);
            }
            ...
            foreach ($auth_list as $k => $v) {
                ...
                if (strpos($v, "admin/") !== false) {
                    ...
                    $auth_list[$k] = $v != '' ? ('admin/' . $v) : $v;
                }
            }
            $auth_list[] = 'admin/index/main';
        }
        if (!in_array($pathUrl, $auth_list, true)) {
            return false;
        } else {
            return true;
        }
