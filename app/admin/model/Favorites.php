...
class Favorites extends Model
{
   ...
   * @param $param
   */
  public function getFavoritesList($where, $param)
  {
    $rows = empty($param['limit']) ? get_config('app.page_size') : $param['limit'];
    $order = empty($param['order']) ? 'id desc' : $param['order'];
    $list = self::where($where)
      ->field('a.*,b.title,b.cover,b.author,b.authorid,b.genre,b.subgenre,b.remark,u.nickname,u.mobile')
      ->alias('a')
   ...
   * @param $param
   */
  public function editFavorites($param)
  {
    try {
   ...
   * @return array
   */
  public function delFavoritesById($id, $type = 0)
  {
    if ($type == 0) {
      //逻辑删除
      try {
