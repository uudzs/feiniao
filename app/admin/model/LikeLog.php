...
class LikeLog extends Model
{
   ...
   * @param $param
   */
  public function getLikeLogList($where, $param)
  {
    $rows = empty($param['limit']) ? get_config('app.page_size') : $param['limit'];
    $order = empty($param['order']) ? 'a.id desc' : $param['order'];
    $list = self::where($where)
      ->field('a.*,u.nickname,b.title,c.title as chapter_title')
      ->alias('a')
      ->join('user u', 'u.id = a.user_id')
      ->join('book b', 'b.id = a.book_id')
      ->join('chapter c', 'c.id = a.chapter_id')
   ...
   * @param $param
   */
  public function editLikeLog($param)
  {
    try {
   ...
   * @return array
   */
  public function delLikeLogById($id, $type = 0)
  {
    if ($type == 0) {
      //逻辑删除
      try {
